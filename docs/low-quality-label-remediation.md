...
| 5 | `知识点@词汇@构词法@转化法` | 70/500 | 已诊断-hold | 已证实：派生、屈折、拼写和普通翻译被混入；T1 tree 末级负向约束无法修复派生误判且会改变控制题路径 | **Conv-Policy-0**：老师裁决 1 条边界题后，验证“词形是否不变”人工/规则字段 | 非 tree 分流字段在独立样本稳定后，才可另开定向重标实验 |
...
|---|---|---|---|---|
| 转化法 | 历史标签混入派生、屈折、拼写、普通翻译；tree 对派生误判无方向性修复，且控制题发生退化 | `hold` | 先取得 `3348636953588649985` 的老师裁决，再验证“词形是否不变”字段 | 该字段在独立样本稳定后，才可另开非 tree 的定向重标实验 |
...
**转化法后续处置改为 `hold`**：保留已审核的 51 条候选审核证据，但停止 whole-tree 在该标签上的批量 replacement。下一项可检验工作不是 DS tree，而是由老师裁决 `3348636953588649985` 后，将“词形是否不变”作为可审计的人工/规则分流字段；只有能在独立样本上证明该字段可靠，才另开非 tree 的定向重标实验。
...
6. **现在进行时的肯否疑 / NP1**：向老师提交 `3125582210285723648`、`2139891546877726720`；裁决后构造 36 条盲审包，DS 恢复后做原/压缩释义对照。
7. **谓语 / Predicate-1**：向老师提交 `2785362399957475328`；裁决后构造 36 条盲审包，DS 恢复后做原/压缩释义对照。
8. **及物动词 / V1**：向老师提交第 5.5 节的 7 条边界题；收到裁决前不生成任何清洗 patch。
9. **过去进行时的肯否疑 / PP1**：向老师提交 `3105096983930556416`；裁决后先构造 36 条盲审包，DS 恢复后再做原/压缩释义对照。
10. **转化法 / Conv-Policy-0**：T1/T1.1 已失败，停止此标签 whole-tree；向老师提交 `3348636953588649985`，之后验证“词形是否不变”的人工/规则分流字段。
11. **其余 P0**：严格按第 3.1 节顺序，每次只启动一个 `*-0` 离线分诊；完成一个标签的“根因 + 唯一下一实验 + 门禁”后才开下一个。
12. **混合词性 M1**：已有盲审包，可与上述离线分诊并行；仍禁止全量。
13. **介词辨析 P2 与音/形/义填空族 P1**：只在对应 P0 不阻塞时运行 route / prompt 对照；每个末级标签单独验收。
