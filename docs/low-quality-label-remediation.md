...
> 2026-09-01 起新增无人值守实验主线：先验证末级标签三分类稳定性，再对稳定 `non_target` 运行动态相邻叶 resolver；所有不稳定结果自动 `hold`。完整契约、门禁和命令见 [劣质知识点释义稳定性与动态相邻叶实验](low-quality-label-stability-dynamic-leaf.md)。本工作台中的旧人工抽检门禁继续作为历史证据，不再是新实验必须新增的步骤。
...
| 22 | `知识点@语法词法@名词@集合名词` | 104/377 | 已完成 Codex 离线初诊-hold | 已证实：必须是集合名词作主语且整体/成员意义直接决定谓语单复数；普通复数、`sheep`、不可数 `hair/furniture`、`staff` 作宾语及姓氏复数被混入 | **Noun-Collective-1**：复核 Codex 的 377 条分层信号，先处理边界和冲突簇（第 30.3 节） | 只有分层复核与独立抽 60 条一致后，才可生成 patch candidate |
| 23 | `知识点@语法句法@并列句@含and并列复合句` | 142/500 | 已完成 Codex 离线初诊-hold | 已证实：DS 把并列句结构过窄地限制为显式两个独立分句/深层时态规则；共享主语并列谓语、时态共现和连词成句中的真实结构可能被漏判；名词短语并列、仅连词词义、祈使句+and及题面缺失被混入 | **Syntax-And-1**：按结构直接参与作答、并列谓语/分句、祈使句+and、仅词义/缺题面分层复核（第 31.3 节） | 先冻结结构与共标 policy，再决定是否生成 patch candidate；不得按 route 一刀切 |
| 24 | `知识点@语法词法@动词@实义动词@不及物动词` | 144/500 | 已完成 Codex 离线初诊-hold（与 V1 成对） | 已证实：DS 对自然出现的不及物动词大多能排除，但漏掉 `dress in`、`arrive there` 等及/不及物结构对比；只考时态、拼写、情态或固定短语的自然出现不应保留 | **Intransitive-1**：与 V1 使用同一套及/不及物边界做成对复核（第 34.3 节） | V1 七条边界裁决并与本标签规则对称后，才可生成 patch candidate |
| 25 | `知识点@语法句法@简单句@主+谓+宾` | 148/500 | 已完成 Codex 离线初诊-hold | 已证实：DS 将“必须直接问句型”当成硬门槛，漏掉真实 SVO 共标；同时混入 SVOC、双宾语、介词短语和仅词义/固定搭配 | **Syntax-SVO-1**：按显式句型、真实 SVO 共标、SVOC/双宾语/介词短语、缺题面分层复核（第 32.3 节） | 先冻结结构共标 policy，再决定是否生成 patch candidate；不得按主考点一刀切 |
| 26 | `知识点@语用@特征@服饰` | 150/500 | 已完成 Codex 离线初诊-hold | 已证实：购物、价格、尺码、颜色等服饰对象可与其他语用标签共标；父题背景、归属/职业/致谢/建议及缺音频被混入 | **Prag-Clothing-1**：按服饰对象直接参与作答、购物共标、父题背景、缺材料分层复核（第 33.3 节） | 先冻结“服饰对象共标”规则，再生成 patch candidate；不得按购物或听力 route 一刀切 |
...
目标标签为 `知识点@语法词法@名词@集合名词`。本轮直接读取并检查源包全部 377 条：JSONL 可解析、question_id 唯一、标签一致；mentor DS 的 `llm_match` 只作为历史证据，不作为真值。本次网页 GPT 的 128 条子样本和 24 条历史校准也只作边界参考，不能外推为 377 条全量结论：
...
#### Codex source-first 离线分诊信号（377 条全包）

以下是根据 CSV 必要条件、题面/答案/解析是否提供集合主语一致证据，以及源记录中的历史理由抽取的**分诊信号**，不是已发布标签：

| 历史 DS 切片 | 保留候选信号 | 删除候选信号 | 边界/待核 | 处理含义 |
|---|---:|---:|---:|---|
| `llm_match=true`（104） | 86 | 3 | 15 | 86 条明确提到集合名词作主语及谓语一致；3 条理由明确显示仅拼写/普通名词；14 条涉及 `hair/staff/furniture/population` 等边界词，1 条理由不足 |
| `llm_match=false`（273） | 48 | 218 | 7 | 48 条理由仍出现结构性主谓一致证据，需逐条确认是否为 DS 漏判；218 条理由明确是拼写、普通复数、所有格、宾语或其他考点；7 条题面泛化/信息不足 |
| 全源包 | 134 | 221 | 22 | 只能作为下一轮人工/规则复核队列，不能直接写回 source |

CSV 列出的常见 route（父题单选、父题语法填空/单词拼写、复合题语法选择）共 270/377；其余 107 条不是自动删除条件，只作为边界诊断层。Codex 初诊的第一原则是内容优先：即使 route 不在“常见题型”列表，只要题面确实让集合主语的整体/成员意义决定谓语形式，仍保留候选。
...
1. 先处理 Codex 分诊信号中的 22 条边界/待核（`hair/staff/furniture/population`、泛化题面和来源理由不足），再复核 48 条 DS false 结构候选；218 条明确非目标只登记为删除候选，不直接删除。
2. 对 86 条强保留候选和 48 条结构候选按“整体/成员意义”“本身表复数”“集合名词作宾语/普通复数”分层，生成可审计的人工核验清单；不得把历史 `llm_reason` 直接当作 Codex 结论。
3. 复核标准必须同时满足“集合名词作主语”和“谓语单复数由其意义决定”；只考拼写、普通词义、所有格、普通复数或集合名词作宾语不保留。
4. 分层复核与独立抽 60 条一致后，才生成待确认 patch candidate；在此之前不修改 source、不批量删除 218 条、不把 86 条直接发布为 released silver。

## 31. P1：含 and 并列复合句

### 31.1 事实画像与可审计证据

目标标签为 `知识点@语法句法@并列句@含and并列复合句`。本轮直接读取源包全部 500 条：JSONL 可解析、question_id 唯一、标签一致；mentor DS 的 `llm_match` 仅作为历史信号：

```text
english-knowledge-tagger-runtime/知识点_语法句法_并列句_含and并列复合句.jsonl
```

| 证据切片 | DS match | DS mismatch | 解读 |
|---|---:|---:|---|
| mentor 源包 500 条 | 142 | 358 | 只有 DS 初筛，不是真实标签准确率 |
| `child × 完形填空 × 语法选择` | 27 | 153 | 数量最大，既有并列结构，也有仅连词/词组选择，不能仅按 route 删除 |
| `child × 复合题 × 语法选择` | 11 | 121 | 复合题小题需看当前空格是否实际依赖 and 结构，不能继承父题背景 |
| `parent × 填空题 × 翻译题` | 39 | 11 | 最接近 CSV 常见题型，但仍需区分结构考查和普通翻译 |
| `parent × 填空题 × 完成句子` | 29 | 17 | 可保留真实并列结构，也可能只是连词/词形填空 |
| `parent × 单选题 × 选择题` | 17 | 26 | 需区分“选择 and 词义”与“and 句法结构” |

已有 24 条完整题面校准样本可作为边界参考：DS true 中 11 条保留、1 条应删除；DS false 中 2 条应保留、10 条删除正确。该小样本显示 DS 对明确结构总体可用，但不能外推为 500 条全量结论。

### 31.2 根因、处理结论与禁止动作

CSV 的必要条件不是“必须出现两个完整主语+谓语且必须考时态一致”，而是 `and` 连接的并列结构实际参与作答；同一主语下的并列谓语、相同主语/谓语省略、并列句时态承接以及翻译或完成句子中由中文“和/而且/一边……一边……”触发的 `and` 都可能保留。相反，仅有名词短语并列、仅考 `and` 词义、`and` 已给出且空格考固定搭配、以及更具体的祈使句+and+陈述句，应分流或另行共标。

本轮主要发现是 DS 的**结构门槛过窄**：大量 mismatch 理由承认存在两个分句或并列谓语，却因没有显式“时态一致/省略主语”而改判为并列连词；同时，部分 match 只依据出现 `and` 或普通并列成分，存在过标。当前处置为 `hold`。禁止：把 358 条 DS mismatch 全部删除；把 142 条 match 全部保留；按“完形/复合题”路由自动删除；把所有 `and` 出现都当作本标签。

### 31.3 Syntax-And-1：并列结构边界分层复核

1. 从 358 条 mismatch 中按“两个完整分句但 DS 以词义为由拒绝”“共享主语并列谓语”“翻译/完成句子结构”“仅名词/动词短语并列”分层；从 142 条 match 中抽取“仅连词选择、祈使句+and、写作/题面泛化”反例。
2. 形成至多 `60` 条删除候选和 `12` 条保留控制，Codex 逐条只判断一个问题：`and` 连接的结构或主语/谓语省略是否直接决定答案；不以题型名称作为充分条件。
3. 明确区分三种结果：`keep_structure`（结构直接参与作答）、`keep_co_label`（and 结构真实存在但另有主要考点，允许共标）、`remove_lexical_or_phrase`（仅词义/短语并列）以及 `hold_missing`（题面不足）。祈使句+and 若有专门标签，记录为替代/共标候选，不静默删除。
4. 分层规则经老师或独立抽检确认后，再对同质 route 扩展；在此之前不生成 patch、不改 source，也不把 24 条校准样本当作 released silver。

## 32. P1：主+谓+宾

### 32.1 事实画像与可审计证据

目标标签为 `知识点@语法句法@简单句@主+谓+宾`。本轮直接读取源包全部 500 条：JSONL 可解析、question_id 唯一、标签一致；mentor DS 的 `llm_match` 仅作为历史信号：

```text
english-knowledge-tagger-runtime/知识点_语法句法_简单句_主+谓+宾.jsonl
```

| 证据切片 | DS match | DS mismatch | 解读 |
|---|---:|---:|---|
| mentor 源包 500 条 | 148 | 352 | 只有 DS 初筛，不是真实标签准确率 |
| `parent × 填空题 × 完成句子` | 31 | 109 | 最大 route；连词成句/完成句子中既有结构题，也有普通词形题 |
| `parent × 单选题 × 选择题` | 86 | 48 | 直接句型识别较集中，但仍混有宾语格、SVOC 和固定搭配 |
| `child × 完形填空 × 语法选择` | 0 | 47 | 小题需按当前空格是否依赖 SVO 判定，不能继承大题结构 |
| `parent × 填空题 × 翻译题` | 3 | 40 | 完整句子常真实形成 SVO，但是否作为共标需看题面是否依赖该结构 |
| `parent × 主观题 × 翻译题` | 1 | 15 | 只凭翻译 route 不自动保留或删除 |

已有 24 条完整题面校准样本可作边界参考：DS true 中 11 条保留、1 条删除；DS false 中 9 条应保留、1 条删除、2 条因题面不足为 `uncertain`。该样本显示 DS true 较稳定，但对真实 SVO 共标存在明显漏判，不能外推为全量准确率。

### 32.2 根因、处理结论与禁止动作

CSV 的必要条件是句子实际形成 `S+V+O`，其中宾语可为名词、宾格代词、动名词或不定式；题目可以直接判断句型，也可以在连词成句、翻译、时态/词形题中让及物动词及其宾语结构直接参与作答。`S+V+O+C`、双宾语、`to/in/about` 等介词短语、仅宾语格/词义/固定搭配而结构不决定答案的题，应分流或仅作其他标签共标。

本轮主要发现是 DS 把“不是直接句型识别”误当成排除条件，导致动名词、代词、名词宾语和真实 SVO 共标大量进入 mismatch；另一方面，部分 match 只是看到及物动词或名词宾语，未确认是否为简单 SVO（例如 SVOC 或方向介词短语）。当前处置为 `hold`。禁止：把 352 条 DS mismatch 全部删除；把 148 条 match 全部保留；把所有含及物动词的句子自动标为 SVO；把复合题父题结构继承给小题。

### 32.3 Syntax-SVO-1：主谓宾结构与共标边界复核

1. 从 352 条 mismatch 中按“真实 SVO 但非直接句型题”“动名词/不定式作宾语”“宾格代词/名词作宾语”“翻译或时态题共标”分层抽取；从 148 条 match 中抽取 SVOC、双宾语、介词短语和仅词义/固定搭配反例。
2. 形成至多 `60` 条删除/漏判候选和 `12` 条保留控制，Codex 逐条判断：`S+V+O` 是否真实存在，且该结构是否直接参与答案；不以题型名称或“主考点不是 SVO”作为充分删除理由。
3. 结果分为 `keep_structure`（明确句型/结构决定答案）、`keep_co_label`（真实 SVO 参与作答且允许共标）、`remove_other_structure`（SVOC/双宾语/介词短语）、`remove_lexical_only`（词义/固定搭配）及 `hold_missing`。
4. 分层规则经小批复核确认后，才可对同质 route 生成 patch candidate；在此之前不改 source、不批量删除 mismatch，也不把 24 条校准样本直接发布为 silver。

## 33. P1：服饰

### 33.1 事实画像与可审计证据

目标标签为 `知识点@语用@特征@服饰`。本轮直接读取源包全部 500 条：JSONL 可解析、question_id 唯一、标签一致；mentor DS 的 `llm_match` 仅作为历史信号：

```text
english-knowledge-tagger-runtime/知识点_语用_特征_服饰.jsonl
```

| 证据切片 | DS match | DS mismatch | 解读 |
|---|---:|---:|---|
| mentor 源包 500 条 | 150 | 350 | 只有 DS 初筛，不是真实标签准确率 |
| `child × 复合题 × 听力单选` | 50 | 149 | 子题必须看当前问题是否依赖服饰信息，不能继承父题衣物背景 |
| `parent × 单选题 × 听力单选` | 77 | 80 | 服饰对象、颜色、尺码、价格和购物共标混在一起 |
| `parent × 复合题 × 听力单选` | 5 | 59 | 缺小题/音频时大量无法判断，不能直接判 false |
| `parent × 填空题 × 补全对话` | 4 | 19 | 需区分服饰相关对话和纯购物/词形填空 |

已有 24 条完整题面校准样本可作边界参考：DS true 12 条均保留；DS false 中 7 条应保留、4 条删除、1 条因音频缺失为 `uncertain`。该样本说明 DS 会把“购物/价格/尺码题中的服饰对象”错误当成互斥标签，也会把父题服饰背景误传给其他小题。

### 33.2 根因、处理结论与禁止动作

CSV 对服饰的定义覆盖衣物名称、穿着描述、颜色/尺码/材质、试穿购买、价格和搭配建议等；这些可以与购物、价格、规格、时间或建议标签共标。只有当前小题的衣物对象或穿着信息实际参与答案时才保留；父题提到衣物但当前小题问职业、归属、外貌、致谢或其他信息，不应继承。听力/图像/复合题缺少决定性材料时必须 `hold`。

本轮主要问题是 DS 的两种相反偏差并存：一方面将服饰与购物、价格、尺码、词形变化机械互斥，造成漏标；另一方面又把父题背景或“woman in red”等修饰语当成当前服饰考点，造成过标。当前处置为 `hold`。禁止：把 350 条 DS mismatch 全部删除；把 150 条 match 全部保留；按购物或听力题型自动决定；把 64 条复合题父题背景传给小题。

### 33.3 Prag-Clothing-1：服饰对象与共标边界复核

1. 从 DS mismatch 中按“衣物对象+购物/价格/尺码/颜色共标”“明确穿着问答”“仅购物无衣物考点”“父题背景传染”分层抽取；从 DS match 中抽取归属、职业、致谢、建议、外貌和词形主考反例。
2. 形成至多 `60` 条删除/漏判候选和 `12` 条保留控制，Codex 逐条判断：当前题目是否需要衣物/穿着/材质/规格信息才能得到答案；不因另有购物、价格或建议主标签而删除。
3. 结果分为 `keep_clothing`（服饰直接决定答案）、`keep_co_label`（服饰与购物/价格/规格等共标）、`remove_parent_background`（服饰只在父题或修饰语背景）、`remove_other_feature`（职业/归属/致谢/外貌等）及 `hold_missing`。
4. 分层规则稳定后，再对同质 route 扩展并独立抽 60 条复核；在此之前不改 source、不批量删除 mismatch，也不把缺音频样本强行补标。

## 34. P1：不及物动词

### 34.1 事实画像与可审计证据

目标标签为 `知识点@语法词法@动词@实义动词@不及物动词`。本轮直接读取源包全部 500 条：JSONL 可解析、question_id 唯一、标签一致；mentor DS 的 `llm_match` 仅作为历史信号：

```text
english-knowledge-tagger-runtime/知识点_语法词法_动词_实义动词_不及物动词.jsonl
```

| 证据切片 | DS match | DS mismatch | 解读 |
|---|---:|---:|---|
| mentor 源包 500 条 | 144 | 356 | 只有 DS 初筛，不是真实标签准确率 |
| `parent × 单选题 × 选择题` | 85 | 123 | 及/不及物对比、介词搭配和普通词义题混在一起 |
| `parent × 填空题 × 单词拼写` | 40 | 128 | 大量只是拼写/时态，动词不及物性质未必参与答案 |
| `parent × 填空题 × 完成句子` | 7 | 28 | 需看是否通过介词或宾语结构约束答案 |
| `child × 复合题 × 语法选择` | 2 | 14 | 只能看当前小题，不能继承复合题父题标签 |

已有 24 条完整题面校准样本可作边界参考：DS true 12 条全部保留；DS false 中 2 条（`dress` 及/不及物搭配、`arrive there` 结构）应保留，10 条删除正确，false 错判率 `2/12 = 16.7%`。该结果说明 DS 主要问题是漏掉结构对比，不是大规模放行自然出现的动词。

### 34.2 根因、处理结论与禁止动作

可保留的题必须有可观察的结构约束：动词本身不直接接名词/代词、必须借助 `to/at/in` 等介词、不能构成被动，或题目直接比较及物/不及物用法（如 `raise/rise`、`hear/listen`、`reach/arrive`、`dress/dress in`）。仅因为 `appear/fall/come/arrive` 出现在句中，而题目实际只考时态、拼写、情态动词或固定短语，不应单独标注。

本轮确认 DS 的漏判与 V1 对称：它常把“题目不是直接词性题”当成排除条件，却漏掉 `dress in`、`arrive there` 等结构确实决定答案的题；反过来，历史 match 也可能把无宾语的自然句子误当专项。当前处置为 `hold`。禁止：把 356 条 DS mismatch 全部删除；把 144 条 match 全部保留；把“动词后没有宾语”作为自动规则；在 V1 边界未冻结前单独生成不及物 replacement。

### 34.3 Intransitive-1：与及物动词成对的结构边界复核

1. 先将 V1 的 7 条老师边界裁决结果写成共享 policy；不及物和及物必须使用同一套“结构直接决定答案”门槛，不能一边按词汇出现保留、一边按词汇出现删除。
2. 从本源包 DS mismatch 中按“及/不及物对比”“介词搭配”“无被动约束”“仅时态/拼写/情态/固定短语”分层，优先复核结构候选；从 DS match 中抽取自然出现和固定短语反例。
3. 形成至多 `60` 条复核候选和 `12` 条控制，Codex 逐条判断：不及物性质是否直接改变选项、答案或句型；记录 `keep_intransitive`、`keep_co_label`、`remove_natural_occurrence`、`remove_other_knowledge`、`hold_missing`。
4. 成对小批规则稳定后，再分别对 V1/I1 同质 route 做全量候选处理和独立 60 条复核；在此之前不改 source、不批量删除、不把 24 条校准样本直接发布为 silver。
...
17. **含 and 并列复合句 / Syntax-And-1**：按结构直接参与作答、并列谓语/分句、祈使句+and、仅词义/缺题面分层复核；构造 `60 remove + 12 keep` 小批。
18. **主+谓+宾 / Syntax-SVO-1**：按显式句型、真实 SVO 共标、SVOC/双宾语/介词短语、缺题面分层复核；构造 `60 remove + 12 keep` 小批。
19. **服饰 / Prag-Clothing-1**：按服饰直接考点、购物共标、父题背景、其他特征和缺材料分层复核；构造 `60 remove + 12 keep` 小批。
20. **不及物动词 / Intransitive-1**：与 V1 共享 policy，按及/不及物对比、介词搭配、无被动和自然出现分层复核；构造 `60 remove + 12 keep` 小批。
21. **一般现在时与现在进行时的区别 / Tense-Contrast-1**：向老师提交 5 条边界题；裁决后构造 `60 remove + 12 keep` 根因分层盲审包，DS 恢复后做原/压缩释义对照。
22. **现在进行时的肯否疑 / NP1**：向老师提交 `3125582210285723648`、`2139891546877726720`；裁决后构造 36 条盲审包，DS 恢复后做原/压缩释义对照。
23. **谓语 / Predicate-1**：向老师提交 `2785362399957475328`；裁决后构造 36 条盲审包，DS 恢复后做原/压缩释义对照。
24. **及物动词 / V1**：向老师提交第 5.5 节的 7 条边界题；收到裁决前不生成任何清洗 patch。
25. **过去进行时的肯否疑 / PP1**：向老师提交 `3105096983930556416`；裁决后先构造 36 条盲审包，DS 恢复后再做原/压缩释义对照。
26. **转化法 / Conv-Policy-0**：T1/T1.1 已失败，停止此标签 whole-tree；使用 conversion-vs-derivation-v1 验证“词形是否不变”的人工/规则分流字段，`3348636953588649985` 继续独立裁决。
27. **其余 P0**：严格按第 3.1 节顺序，每次只启动一个 `*-0` 离线分诊；完成一个标签的“根因 + 唯一下一实验 + 门禁”后才开下一个。
28. **混合词性 M1**：已有盲审包，可与上述离线分诊并行；仍禁止全量。
29. **介词辨析 P2 与音/形/义填空族 P1**：只在对应 P0 不阻塞时运行 route / prompt 对照；每个末级标签单独验收。
