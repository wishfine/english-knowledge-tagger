...
3. **谓语 / Predicate-1**：向老师提交 `2785362399957475328`；裁决后构造 36 条盲审包，DS 恢复后做原/压缩释义对照。
4. **及物动词 / V1**：向老师提交第 5.5 节的 7 条边界题；收到裁决前不生成任何清洗 patch。
5. **过去进行时的肯否疑 / PP1**：向老师提交 `3105096983930556416`；裁决后先构造 36 条盲审包，DS 恢复后再做原/压缩释义对照。
6. **转化法 / T1**：实验包已就绪；DS 恢复后运行 60 条 whole-tree，检查“同形/派生/屈折/翻译”四个边界簇。
7. **其余 P0**：严格按第 3.1 节顺序，每次只启动一个 `*-0` 离线分诊；完成一个标签的“根因 + 唯一下一实验 + 门禁”后才开下一个。
8. **混合词性 M1**：已有盲审包，可与上述离线分诊并行；仍禁止全量。
9. **介词辨析 P2 与音/形/义填空族 P1**：只在对应 P0 不阻塞时运行 route / prompt 对照；每个末级标签单独验收。
