...
| 2 | `知识点@语法词法@动词时态@现在进行时@现在进行时的肯否疑` | 47/500 | 待离线分诊 | 待验证：DS 把 `be + V-ing` 形式考查当作“非唯一主考点” | **Tense-Now-0**：500 条按 route/文本完整性剖析 + 网页 GPT 原始直审并给标签结论 | 根据 false 保留率决定 prompt 对照或 tree，不得直接删标 |
| 3 | `知识点@语篇主题@人与社会@互联通讯` | 53/500 | 已诊断-hold | 强信号：电话/邮箱/电视/网站等关键词被误作“网络社交或信息传递”主题；大量阅读父题不是该主题 | **Theme-1**：403 条 remove 中分层构造 tree 纠错 60 条小包（第 7.3 节） | 仅输出 `relabel_candidate` / `uncovered`，再按候选叶子复核 |
| 4 | `知识点@语法句法@句子成分@谓语` | 60/500 | 待离线分诊 | 待验证：谓语自然出现被误作考点；句子成分可共标规则不清 | **Syntax-Predicate-0**：按选择/填空/改错分层，审“谓语是否直接决定答案” | 决定共标 prompt 或 route quarantine |
...
| 7 | `知识点@语用@时间@顺序` | 19/127 | 待离线分诊 | 待验证：时间顺序表达与普通时间信息/语篇理解混杂 | **Prag-Order-0**：按对话/阅读/听力及上下文完整性各层抽审 | 决定语用场景 policy 或 hold |
| 8 | `知识点@语用@社会交往@争辩` | 57/365 | 待离线分诊 | 待验证：争辩与建议、投诉、请求等近邻语用意图混淆 | **Prag-Argue-0**：对话场景×言语行为分层审核 | 只对稳定言语行为簇做 prompt 对照 |
...
2. **现在进行时的肯否疑 / Tense-Now-0**：不依赖 DS；准备 500 条 route/信息完整性画像和网页 GPT 原始直审包。
3. **及物动词 / V1**：向老师提交第 5.5 节的 7 条边界题；收到裁决前不生成任何清洗 patch。
4. **过去进行时的肯否疑 / PP1**：向老师提交 `3105096983930556416`；裁决后先构造 36 条盲审包，DS 恢复后再做原/压缩释义对照。
5. **转化法 / T1**：实验包已就绪；DS 恢复后运行 60 条 whole-tree，检查“同形/派生/屈折/翻译”四个边界簇。
6. **其余 P0**：严格按第 3.1 节顺序，每次只启动一个 `*-0` 离线分诊；完成一个标签的“根因 + 唯一下一实验 + 门禁”后才开下一个。
7. **混合词性 M1**：已有盲审包，可与上述离线分诊并行；仍禁止全量。
8. **介词辨析 P2 与音/形/义填空族 P1**：只在对应 P0 不阻塞时运行 route / prompt 对照；每个末级标签单独验收。
