...
以下“人工复核”数字均来自已有完整题面样本；它们是当前工作结论，不是对全量真实准确率的估计。完整样本台账仍在更新，因此每次开始新标签前必须重新计算本节的优先级，不能把这里的顺序视为永久队列。
...
| P0 | `知识点@语法词法@主谓一致@意义一致` | 历史初筛 176/500 true；完整复核 true 2/12、false 1/10 应留 | 普通形式一致、`there be`、`each` 等被混入意义一致；正例污染严重 | 禁止 rollout | 先做“意义一致特例”边界诊断 |
| P0 | 结构共标簇：`语法一致`、`含but并列复合句`、`主+系+表` 等 | 完整复核 false 错判率 100%（各可判分母 11--12） | DS 将“不是唯一主考点”错误当作排除结构共标的理由 | 禁止用 false 清洗 | 以 `语法一致` 为代表做共标 prompt/规则对照 |
...
| P2 | `知识点@词汇@词汇辨析@介词（短语）辨析` | 历史初筛 404/500 true；完整复核 true 10/12、false 3/12 应留 | route 硬限制未前置，固定介词搭配有一定 false 漏删 | 未校准 | 题型链路确认后做 route 预过滤与重新校准 |
