...
| [网页 GPT 协作契约](web-gpt-collaboration-contract.md) | 网页 GPT 与 Codex 的分工、盲审 JSONL 输入输出和验收门禁 | 将逐题复核委托给网页 GPT 前 |
...
| [384 个知识点标签总状态台账](knowledge-label-status-ledger-20260904.md) | 汇总全部 384 个标签的 DS 指标、终判状态、排除项和 taxonomy 修正 | 查看全量标签进度或安排下一批处理时 |
...
该 policy 现在明确锁定 `prompt_version=mentor-direct-v1`。无历史标签锚定的最终判别器使用 `final-label-discriminator-v1`，必须在同一 `label × route` 的人工样本上重新校准；在新 policy 冻结前，final evidence 会被 gate 保守地写入 hold。
