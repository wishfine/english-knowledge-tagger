...
> 当前高质量提取的第一门是“逐个现有末级标签的直接判别 + 人审校准 policy”，而不是本文件的 flat 候选池。直接判别的标准化、保守放行与题级组装命令见 [当前数据处理 Loop](current-data-loop.md#21-直接末级标签判别--保守-silver当前高质量提取入口)。本文件描述的是 direct `match=false`、未校准、缺标和候选替换难例的**第二阶段**。
>
> 因而，本文件中的 `allowed_knowledge_prefixes`、retrieval top-k 和同级兄弟预算只限制 flat/tree 的替换候选；它们不能用于否定一个已有标签的直接正向判别，也不能作为未校准标签的自动放行规则。
...
#### 首轮结果（2026-08-26）：不切换 flat 默认策略

首个 `child × 复合题 × 语法选择` 冻结样本中，v0.2 使 20/983 个标签验证项的完整 alternatives 集合真正新增了标签（被动语态 9、宾语从句引导词 7、限制性定语从句 4）。在该 20 条有效子集上，v0.1 和 v0.2 各运行三次，结果为：

| 指标 | v0.1：最多 8 个直接兄弟 | v0.2：全部直接兄弟 |
|---|---:|---:|
| 三次决策一致率 | 20/20（100%） | 20/20（100%） |
| 跨版本一致决策差异 | \- | 0 条 |
| v0.2 三次一致选择新增叶子 | \- | 0 条 |
| 每次 verdict | 18 keep、2 drop | 18 keep、2 drop |
| 平均 wall time（20 条，并发 16） | 27.12 秒 | 29.55 秒（+8.97%） |
| 平均最大 RSS | 41,797 KB | 42,741 KB |

因此，v0.2 在当前 target-centered flat prompt 下没有发现新的 `replace`，却增加了延迟。**决策：v0.1 继续作为 flat 历史标签验证的默认策略；不对 980 条或全量数据放大 v0.2。**

这不是“完整直接兄弟永远无用”的结论：这 20 条的历史标签可能本来就是正确的，而 flat prompt 还把历史标签及其原始释义放在强锚点位置。若后续要重新评估 sibling 覆盖，应先建立人工确认的对抗校准集：历史标签已知错误，正确叶子恰好是 v0.1 缺失、v0.2 新增的同级标签。没有这类金标，不能用 DS 自己的 keep 偏好证明候选策略优劣。
