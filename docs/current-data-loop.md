...
使用 `scripts/repair_parent_context.py`（`parent-context-repair-v3`）生成 `v3_parent_context` 派生源时，只能从
raw 唯一匹配的父题 `stem/options` 补文本上下文，不能复制父题的
`knowledge_points` 或 `question_types`，也不能覆盖 v2 的音频/图片增强。父题上下文
统一按 `题目大题题干：...`、`当前小题题干：...` 的字段格式拼接，插在开头题型/音频元信息
之后、当前小题正文之前；已有 `当前小题选项/解析/答案` 等字段保持原样。修复输出、
...
最终判别器还会记录输入证据完整性。规则侧 `input_precheck.status` 由题面字段、父题上下文、解析/答案和多模态标记确定；使用 `--include-input-status` 时，DS 额外返回 `llm_input_status`。`complete` 表示有直接题面，`analysis_supported` 表示虽缺当前小题题干但解析提供了具体依据，`insufficient`/`ambiguous` 表示不能可靠核验。输入不足或兄弟题映射歧义不等同于标签错误：最终质量快照会把这些正向证据写入 `holds.jsonl`，而不是当成确定负例；`analysis_supported` 单独计数，不能与完整题面混为一谈。
