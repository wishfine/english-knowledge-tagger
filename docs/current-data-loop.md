...
发送给 DS-V4 的内容是：小题题干、选项、答案、解析和必要父题上下文；**源文本中的“题型结构/题型名称”会移除**。每条历史知识点携带自己的原始老师释义；可替换标签从该 route 的允许前缀内检索，默认至多 12 个，再附必要的同级近邻。不能向模型发送整棵 576 标签树。
...
树从老师 CSV 的 active 知识点路径构建；每一步仅显示当前同层候选和 `__NO_MATCH__` 控制项。`__NO_MATCH__` 不是真实“其他”标签，会触发回退和排除失败分支。默认上限为 8 步、2 次回退。
