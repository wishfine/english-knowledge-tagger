...
- `scripts/rank_mentor_verification_report.py`、`scripts/build_mentor_label_rollout_packet.py`、`scripts/validate_mentor_label_rollout.py` 与 `scripts/sample_silver_post_sweep.py`：对 mentor 的 500 条初筛按 Wilson 产量下界排序；用同一 `mentor-direct-v1` prompt 跑一个标签的全量题；由独立 60 条复核决定是否从 preliminary silver 升级。
